    ...
    return (
        <BlockWrapper className="bg-brand-charcoal/50">
            ...
            <div className="relative aspect-[4/5] rounded-[50px] overflow-hidden mb-12 group grayscale hover:grayscale-0 transition-all duration-700 shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/5">
                ...
                <img 
                    src={content.image?.startsWith('assets/') ? `/${content.image}` : (content.image || "/assets/PhotoExpert.jpg")} 
                    alt={content.name || "Expert"} 
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000" 
                />
                <div className="absolute bottom-10 left-10 z-20">
                    <div className="font-mono text-[11px] text-brand-emerald font-black mb-3 uppercase tracking-widest">
                        {content.role || ""}
                    </div>
            ...
            {content.quote && (
                <div className="mb-12 text-center space-y-4">
                    <p className="text-brand-zinc/50 text-sm italic">
                        {content.quote}
                    </p>
                    {content.quote_author && (
                        <div className="text-[10px] text-brand-emerald font-black uppercase tracking-widest">
                            — {content.quote_author}
                        </div>
                    )}
                </div>
            ...
            >
                {content.button_text || "ПОКАЗАТЬ КЕЙСЫ"}
            </button>
        </BlockWrapper>
    );
};