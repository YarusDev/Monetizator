...
interface AntiTargetSettingsProps {
  block: any;
  onChange: (content: any) => void;
}

export const AntiTargetSettings = ({ block, onChange }: AntiTargetSettingsProps) => {
  const content = block.content || {};

  // Normalize items
  const rawItems = content.items || [];
  const items = Array.isArray(rawItems) ? rawItems.map((item: any) => {
    if (typeof item === 'string') return { title: item, description: '', icon: 'XCircle' };
    return {
      title: item.title || '',
      description: item.description || '',
      icon: item.icon || 'XCircle'
    };
  }) : [];
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <SectionHeader 
        title="Анти-цели / Боли" 
        subtitle="Что мы НЕ делаем и для кого НЕ подходит"
        icon={ShieldAlert}
      />
      ...
      <div className="space-y-6 border-t border-white/5 pt-12">
        <div className="flex justify-between items-center mb-4">
          <h4 className="text-[9px] text-brand-emerald font-black uppercase tracking-[0.2em]">Список тезисов</h4>
          <AddButton 
            onClick={() => handleUpdate('items', [...items, { title: 'Новый тезис', description: '', icon: 'XCircle' }])}
            label="Добавить"
          />
        </div>
